...
class InorderTraversal : public TreeTraversal<T> {
  public:
	    ...
	    : root(root)
    {
      stack.push(root);
      // your code here
			typename BinaryTree<T>::Node* curr = root->left;
			while (curr != NULL) {		// get to the bottom left leaf node
				stack.push(curr);
				curr = curr->left;
			}
    }
    ...
    void add(typename BinaryTree<T>::Node *& treeNode) {
      // your code here
			if (treeNode->right != NULL) {		// check the right side
				stack.push(treeNode->right);
				if (treeNode->right->left != NULL) {
					// essentially same as first step of traversal
					typename BinaryTree<T>::Node* curr = treeNode->right->left;
					while (curr != NULL) {		// get to the bottom left leaf node
						stack.push(curr);
						curr = curr->left;
					}
